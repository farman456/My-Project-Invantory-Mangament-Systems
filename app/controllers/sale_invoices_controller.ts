...
export default class SaleInvoicesController {
  ...
  async create(ctx: HttpContext) { try { return sendSuccess('Sale invoice created successfully', await createSaleInvoice(await createSaleInvoiceValidator.validate(ctx.request.body()))) } catch (error) { console.log('Sale invoice creating error', error); return ErrorService.handleError(ctx, error) } }
  async update(ctx: HttpContext) { try { const { saleInvoiceId } = await saleInvoiceIdValidator.validate(ctx.params); const payload = ctx.request.method() === 'PATCH' ? await updateSaleInvoiceValidator.validate(ctx.request.body()) : await createSaleInvoiceValidator.validate(ctx.request.body()); return sendSuccess('Sale invoice updated successfully', await updateSaleInvoice(payload, saleInvoiceId)) } catch (error) { console.log('Sale invoice updating error', error); return ErrorService.handleError(ctx, error) } }
