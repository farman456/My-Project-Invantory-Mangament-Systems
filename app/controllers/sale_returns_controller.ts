...
export default class SaleReturnsController {
  ...
  async create(ctx: HttpContext) { try { return sendSuccess('Sale return created successfully', await createSaleReturn(await createSaleReturnValidator.validate(ctx.request.body()))) } catch (error) { console.log('Sale return creating error', error); return ErrorService.handleError(ctx, error) } }
  async update(ctx: HttpContext) { try { const { saleReturnId } = await saleReturnIdValidator.validate(ctx.params); const payload = ctx.request.method() === 'PATCH' ? await updateSaleReturnValidator.validate(ctx.request.body()) : await createSaleReturnValidator.validate(ctx.request.body()); return sendSuccess('Sale return updated successfully', await updateSaleReturn(payload, saleReturnId)) } catch (error) { console.log('Sale return updating error', error); return ErrorService.handleError(ctx, error) } }
