...
export default class DeliveryVouchersController {
  ...
  async create(ctx: HttpContext) { try { return sendSuccess('Delivery voucher created successfully', await createDeliveryVoucher(await createDeliveryVoucherValidator.validate(ctx.request.body()))) } catch (error) { console.log('Delivery voucher creating error', error); return ErrorService.handleError(ctx, error) } }
  async update(ctx: HttpContext) { try { const { deliveryVoucherId } = await deliveryVoucherIdValidator.validate(ctx.params); const payload = ctx.request.method() === 'PATCH' ? await updateDeliveryVoucherValidator.validate(ctx.request.body()) : await createDeliveryVoucherValidator.validate(ctx.request.body()); return sendSuccess('Delivery voucher updated successfully', await updateDeliveryVoucher(payload, deliveryVoucherId)) } catch (error) { console.log('Delivery voucher updating error', error); return ErrorService.handleError(ctx, error) } }
