...
export default class SaleOrdersController {
  ...
  async create(ctx: HttpContext) { try { return sendSuccess('Sale order created successfully', await createSaleOrder(await createSaleOrderValidator.validate(ctx.request.body()))) } catch (error) { console.log('Sale order creating error', error); return ErrorService.handleError(ctx, error) } }
  async update(ctx: HttpContext) { try { const { saleOrderId } = await saleOrderIdValidator.validate(ctx.params); const payload = ctx.request.method() === 'PATCH' ? await updateSaleOrderValidator.validate(ctx.request.body()) : await createSaleOrderValidator.validate(ctx.request.body()); return sendSuccess('Sale order updated successfully', await updateSaleOrder(payload, saleOrderId)) } catch (error) { console.log('Sale order updating error', error); return ErrorService.handleError(ctx, error) } }
